...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_id'])) {
    ...
    // PARANDUS: Kõik andmed võetakse nüüd korrektselt $_POST massiivist välja
    $mark = $_POST['mark'];
    $model = $_POST['model'];
    $engine = $_POST['engine'];
    $fuel = $_POST['fuel'];
    $price = intval($_POST['price']); // Muudetud täisarvuks turvalisuse huvides
    $description = $_POST['description'];

    $stmt = mysqli_prepare($yhendus,
        "UPDATE cars SET mark=?, model=?, engine=?, fuel=?, price=?, description=? WHERE id=?");
    
    // Parameetrite tüübid vastavad muutujatele: s = string, i = integer (integerid on price ja id)
    mysqli_stmt_bind_param($stmt, "ssssisi",
        $mark, $model, $engine, $fuel, $price, $description, $id);
                ...
                <table class="table align-middle mb-0">
                    ...
                    <form method="POST">
                        ...
                        <td style="width:120px;">
                            <!-- PARANDUS: Asendatud tundmatu muutuja $rida[1] korrektse $auto['mark'] muutujaga -->
                            <img src="https://loremflickr.com/600/350/<?php echo htmlspecialchars($auto['mark']); ?>" 
                                 class="card-img-top img-fluid rounded-3 shadow-sm" 
                                 alt="auto"
                        ...
                        <td>
                            <input type="text" name="mark" class="form-control form-control-sm mb-1"
                                   value="<?php echo htmlspecialchars($auto['mark']); ?>" required>
                            <input type="text" name="model" class="form-control form-control-sm"
                                   value="<?php echo htmlspecialchars($auto['model']); ?>" required>
                        </td>

                        <td>
                            <input type="text" name="engine" class="form-control form-control-sm"
                                   value="<?php echo htmlspecialchars($auto['engine']); ?>">
                        </td>
                    ...
                    <tr>
                        ...
                        <td style="width:200px;">
                            <img src="https://loremflickr.com/200/200/<?php echo htmlspecialchars($auto['mark']); ?>" class="card-img-top" alt="auto">
                        </td>
                        ...
                        <td>
                            <?php echo htmlspecialchars($auto['engine']); ?>
                        </td>
